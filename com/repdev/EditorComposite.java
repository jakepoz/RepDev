...
//							return;
	...
	final public int calcWidth(){
        ...
        int top = txt.getTopPixel();
        int height = txt.getClientArea().height;
        int lastLine = txt.getLineIndex(top + height);
        if (lastLine >= 60){
        	lastLine = lastLine;
        }
        lastLine = txt.getLineCount()+1;
...
//			}
		...
		style.foreground = new Color(Display.getCurrent(), new RGB(105, 105, 105));
		
		int bulletWidth = calcWidth();

		style.metrics = new org.eclipse.swt.graphics.GlyphMetrics(0, 0, bulletWidth);
		style.start = 1;
