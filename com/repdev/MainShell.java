...
public class MainShell {
	...
	bottom.addControlListener(new ControlAdapter() {
		...
		if (bottom.getSize().y < MIN_COMP_SIZE) {
		    frmSashHoriz.top = new FormAttachment(right.getSize().y
			    - MIN_COMP_SIZE, right.getSize().y, 0);
		    right.layout();
		}
	    }
	});
	...
	for (CTabItem c : mainfolder.getItems()) {
	    if (c.getData("seq") != null && (Sequence) c.getData("seq") == seq
		    && c.getData("sym") != null
		    && ((Integer) c.getData("sym")) == sym) {
	...
	if ((dir = dialog.open()) != null) {
		...
		if (current.getData() instanceof String
			&& ((String) current.getData()).equals(dir))
		    exists = true;
	    }
	...
	if ((result & RemFileShell.OK) > 0
		&& (result & RemFileShell.DELETE) == 0) {
	    proj.removeFile(file, false);
	    cur.dispose();
	} else if ((result & RemFileShell.OK) > 0
		&& (result & RemFileShell.DELETE) > 0) {
	...
	target.addDropListener(new DropTargetAdapter() {
		...
		if (event.item == null) {
		    ...
		    if ((getTreeDir(root) == null
			    || getTreeDir(dragSourceItems[0]) == null || getTreeDir(root).equals(getTreeDir(dragSourceItems[0])))
			    && getTreeSym(root) == getTreeSym(dragSourceItems[0]))
			overwrite = RepeatOperationShell.APPLY_TO_ALL
				| RepeatOperationShell.NO;
		    else
			overwrite = RepeatOperationShell.ASK_TO_ALL
				| RepeatOperationShell.YES;
			...
			for (TreeItem item : dragSourceItems) {
				...
				if (RepDevMain.SYMITAR_SESSIONS.get(getTreeSym(root)) != null
					&& RepDevMain.SYMITAR_SESSIONS.get(getTreeSym(root)).isConnected())
			    ...
			    if (exists
				    && (overwrite & RepeatOperationShell.ASK_TO_ALL) != 0) {
				RepeatOperationShell dialog = new RepeatOperationShell(shell, "File "
					+ destination.getName()
					+ " already exits at the destination. Overwrite?");
			    ...
			    if (error == SessionError.NONE
				    && root.getData() instanceof Project)
	...
	final MenuItem symLogoff;
	if (RepDevMain.DEVELOPER) {
	    symLogoff = new MenuItem(treeMenu, SWT.NONE);
		...
		public void widgetSelected(SelectionEvent e) {
		    ...
		    dialog.setMessage("Are you sure you want to logoff of this Sym (sym "
			    + sym
			    + ") ?"
	...
	treeMenu.addMenuListener(new MenuListener() {
		...
		if (tree.getSelectionCount() == 2
			&& tree.getSelection()[0].getData() instanceof SymitarFile
			&& tree.getSelection()[0].getData() instanceof SymitarFile)
		    compareFile.setEnabled(true);
		else
		    compareFile.setEnabled(false);

		if (!(tree.getSelection()[0].getData() instanceof Integer || tree.getSelection()[0].getData() instanceof String)
			&& tree.getSelectionCount() == 1) {
		    importFilem.setEnabled(true);
		    newProjectFile.setEnabled(true);
		}

		if (tree.getSelection()[0].getData() instanceof Project
			&& tree.getSelectionCount() == 1) {
		...
		if (tree.getSelectionCount() == 1
			&& tree.getSelection()[0].getData() instanceof SymitarFile
			&& ((SymitarFile) tree.getSelection()[0].getData()).getType() == FileType.REPGEN
	...
	tree.addListener(SWT.Expand, new Listener() {
		...
		if (root.getData() instanceof Integer
			|| root.getData() instanceof String) {
		    if (root.getData() instanceof Integer) {
	...
	if (item.getData() instanceof String
		|| item.getData() instanceof Integer)
	    return;
	...
	if (mainfolder.getSelection() != null
		&& mainfolder.getSelection().getControl() instanceof TabTextView) {
	...
	mainfolder.addSelectionListener(new SelectionAdapter() {
		...
		if (mainfolder.getSelection() != null
			...
			&& mainfolder.getSelection().getControl() instanceof ReportComposite)
	...
	if (item != null && item.getData("modified") != null
		&& ((Boolean) item.getData("modified"))) {
	    MessageBox dialog = new MessageBox(shell, SWT.ICON_QUESTION
		...
		dialog.setMessage("The file '"
			...
			+ "' on Sym "
			+ item.getData("loc")
			+ " has been modified, do you want to save it before closing it?");
	    else
		dialog.setMessage("The file '"
			+ item.getData("file")
			+ "' in directory "
	...
	if (mainfolder.getSelection().getControl() instanceof EditorComposite)
		...
		if (tItem.getData("file").equals(mainfolder.getSelection().getData("file"))
			&& tItem.getData("sym").equals(mainfolder.getSelection().getData("sym")))
		    tItem.dispose();
	...
	tblErrors.addSelectionListener(new SelectionAdapter() {
		...
		if (error.getLine() >= 0 && editor != null) {
		    ...
		    try {
			editor.getStyledText().setCaretOffset(Math.min(editor.getStyledText().getOffsetAtLine(Math.max(0, error.getLine() - 1))
				+ Math.max(0, error.getCol() - 1), editor.getStyledText().getCharCount() - 1));
	...
	fileSaveAs.addSelectionListener(new SelectionAdapter() {
		...
		if (mainfolder.getSelection() != null
			...
			// clear to the user
			for (CTabItem item : mainfolder.getItems())
			    if (item.getData("file") != null
				    && item.getData("file").equals(result.get(0)))
				item.dispose();
	...
	fileMenu.addMenuListener(new MenuListener() {
		...
		fileSave.setEnabled(mainfolder.getSelection() != null
			&& mainfolder.getSelection().getControl() instanceof EditorComposite);
		fileSaveAs.setEnabled(mainfolder.getSelection() != null
			&& mainfolder.getSelection().getControl() instanceof EditorComposite);
		...
		for (final SymitarFile file : Config.getRecentFiles()) {
			...
			public void widgetSelected(SelectionEvent e) {
			    if (!file.isLocal()
				    && !RepDevMain.SYMITAR_SESSIONS.get(file.getSym()).isConnected())
	...
	if (mainfolder.getSelection() != null
		...
		options.footer = "\t\t<page>";
		options.jobName = "RepDev - "
			+ mainfolder.getSelection().getText();
